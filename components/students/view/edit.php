...
<div id="content-top">
	<div id="top-menu">
		<ul class="line-menu">
			<li><a href="<?= $helper->url ?>&id=<?= $id ?>">Показать</a></li>
		...
		<form action="<?= $url ?>" method="POST">
			<table id="form">
				...
				<tr>
					...
					<td class="">+7
						(<input id="phone-code" type="text" name="phone-code" size="3" value="<?= $phoneCode ? $phoneCode : '' ?>" />)
						<input id="phone-number" type="text" name="phone-number" size="10" value="<?= $phoneNumber ? $phoneNumber : '' ?>" />
					</td>
					...
					<td class="input">
						<select id="mother" name="mother" size="1">
							...
							<? foreach ($listMother as $item) : ?>
								<option value="<?= $item->id ?>" <? if ($mother == $item->id) : ?> selected="" <? endif; ?>><?= $item->surname ?> <?= $item->name ?> <?= $item->patronymic ?></option>
							<? endforeach; ?>
						</select>
					</td>
					...
					<td class="input">
						<select id="father" name="father" size="1">
							...
							<? foreach ($listFather as $item) : ?>
								<option value="<?= $item->id ?>" <? if ($father == $item->id) : ?> selected="" <? endif; ?>><?= $item->surname ?> <?= $item->name ?> <?= $item->patronymic ?></option>
							<? endforeach; ?>
						</select>
					</td>
				...
				<tr>
					<td></td>
					<td colspan="2"><input id="editStudent" type="submit" name="editStudent" value="Изменить" /></td>
				</tr>
			</table>
		</form>
	<? endif; ?>
